...
class Logger {
  ...
  _datetime() {
    let d = new Date();
    d = ['' + d.getFullYear(), '0' + (d.getMonth() + 1), '0' + d.getDate(), '0' + d.getHours(), '0' + d.getMinutes(), '0' + d.getSeconds()].map(component => component.slice(-2)); // take last 2 digits of every component
...
class ResultSeriesType {
  ...
  static contains(code) {
    return Object.values(ResultSeriesType.typeCode)
      .map(a => a.code)
      .includes(code);
  }
