...
/**
 *  
 * GeneTopCountStatsParser is a {@Link Parser} for {@Link GeneTopCountStatsResult}
 * 
 * @extends {Parser}
 */
class ImmuneDBGeneTopCountStatsParser extends Parser {
    ...
    preparse(properties) {
        ...
        if (this.#_percentage){
            parsedProperties.setYLabel("Percentage (copies in repertoire)");
            ...
            if(messageObjectStatisticsArray.length > 0) {
                ...
                for (let index = 0; index < messageObjectStatisticsArray.length; index++) {
                    ...
                    for (let i = 0; i < statisticDataArray.length; i++) {
                        ...
                        let dataItem = new ResultSeriesDataItem().setName(repId).setY(this.#_percentage?value/statisticTotal*100:value); 
...
export {ImmuneDBStatsParserConstants, ImmuneDbCloneCountParser, ImmuneDBGeneTopCountStatsParser};