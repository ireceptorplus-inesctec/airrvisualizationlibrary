import {Logger, ResultSeriesType, Common, ChartType, DebugTimer} from './common.js';
...
/**
 *
 * ImmuneDBGeneUsageStatsParser is a ImmuneDB {@Link Parser} for {@Link GeneStatsResult} without drilldown capabilities
 *
 * @extends {Parser}
 */
class ImmuneDBGeneUsageStatsParser extends Parser {
  #_logger;
  #_multipleSeries;
  #_sort;
  #_percentage;
  ...
  /**
   * @description Creates an instance of ImmuneDBGeneUsageStatsParser.
   */
  constructor() {
    super();
    this.#_logger = new Logger('ImmuneDBGeneUsageStatsParser');
    this.#_logger.debug('Constructor.');

    this.#_multipleSeries = false;
    this.#_series = [];
    this.#_seriesByRepertoire = {};
    this.#_keysByRepertoire = {};
   ...
   * @param {Array} keys
   * @memberof ImmuneDBGeneUsageStatsParser
   */
  _initializeRepertoire(series) {
    let keys = this.#_dataKeys;
    let repertoireId = series.repertoireId;
    if (!this.#_seriesByRepertoire[repertoireId]) {
      this.#_seriesByRepertoire[repertoireId] = series;
      let keysForRepertoire = {};
      keys.forEach(key => (keysForRepertoire[key] = 0));
      this.#_keysByRepertoire[repertoireId] = keysForRepertoire;
    }
  }

  _addToSeries(repertoireId, dataItem) {
    let key = dataItem.getName();
    this.#_keysByRepertoire[repertoireId][key] = 1;
    this.#_seriesByRepertoire[repertoireId].data.push(dataItem);
  }

  _coalescAndSort() {
    // Não está a funcionar.
    const repertoires = Object.keys(this.#_seriesByRepertoire);
    repertoires.forEach((repertoireId, index) => {
  ...
  preparse(properties) {
    ...
    //Create a set with distinct keys
    let jsonpathQueryString = '$..' + ImmuneDBStatsParserConstants.KEY;
    let distinctDataKey = [...new Set(JSONPath(jsonpathQueryString, this.#_data))];
    // Need this array to be sorted for later when pl0tting series on the chart.
    distinctDataKey.sort((a, b) => b.localeCompare(a));
    this.#_dataKeys = distinctDataKey;
    //Create a set with distinct repertoire_ids
    jsonpathQueryString = '$..' + ImmuneDBStatsParserConstants.REPERTOIRE_ID;
    let distinctDataRepertoireIds = [...new Set(JSONPath(jsonpathQueryString, this.#_data))];
    // Need this array to be sorted for later when pl0tting series on the chart.
    distinctDataRepertoireIds.sort((a, b) => b.localeCompare(a));

    if (distinctDataRepertoireIds.length > 1) {
      // We have a multiple keys to plot
      this.#_multipleSeries = true;
      //Check color array for sufficient colors for the number of keys.
      if (this.#_seriesColors.length < distinctDataRepertoireIds.length) {
        this.#_logger.error('Not enough colors set for the amount of repertoires. Please increase the number of colors in seriesColor array.');
        throw 'Not enough colors set for the amount of repertoires. Please increase the number of colors in seriesColor array.';
      }
      colorIndexJumper = Math.floor((this.#_seriesColors.length - 1) / (distinctDataRepertoireIds.length - 1));
      parsedProperties.setDraw3D(true);
    }

    //Prepare a ResultSeries for each of the existing Repertoire_ids
    for (let index = 0; index < distinctDataRepertoireIds.length; index++) {
      const repertoireId = distinctDataRepertoireIds[index];
      const seriesName = repertoireId;
      let color = this.#_seriesColors[colorIndex];
      colorIndex += colorIndexJumper;
      ...
      let series = new ResultSeries()
        ...
        .setSampleProcessingId(undefined) //irrelevant for this structure
        .setDataProcessingId(undefined) //irrelevant for this structure
        .setId(repertoireId) //irrelevant for this structure
        .setName(seriesName)
        .setFieldName(undefined) //irrelevant for this structure
        .setColor(color);
      this._initializeRepertoire(series);
    }
  }
    ...
    for (let j = 0; j < messageArrayLength; j++) {
      ...
      if (messageArrayObject[StatsParserConstants.STATISTICS].length > 0) {
        ...
        for (let i = 0; i < firstObject[StatsParserConstants.DATA].length; i++) {
          ...
          let value = dataObject[StatsParserConstants.VALUE];
          let key = dataObject[StatsParserConstants.KEY];
          let dataItem = new ResultSeriesDataItem().setName(key).setY(this.#_percentage ? (value / totalUsageCount) * 100 : value);
          this._addToSeries(repID, dataItem);
        }
      } else {
    ...
    for (let i = 0; i < studies.length; i++) {
      ...
      for (const key in studiesDict[study]) {
        if (studiesDict[study].hasOwnProperty(key)) {
          const value = Common.roundTo(
            studiesDict[study][key].reduce((a, b) => a + b, 0),
            4,
          );
          let dataItemTop = new ResultSeriesDataItem()
            .setName(study + '-' + key)
            .setY(value)
            .setColor(colorTopByStudy[study]);
          seriesTopData.push(dataItemTop);
          let dataItemRest = new ResultSeriesDataItem()
            .setName(study + '-' + key)
            .setY(1 - value)
            .setColor(colorRest);
...
export {ImmuneDBStatsParserConstants, ImmuneDBFunctionalityCountStatsParser, ImmuneDbCloneCountParser, ImmuneDBGeneTopCountStatsParser, ImmuneDBGeneUsageStatsParser};
